...
const BoschMetadataProcessor = function (_cfg) {
    ...
    function processMsg(msg) {
        ...
        datatodownload.push([objs, Date.now()])
        if (count >= 500)
        {
            const jsonMsg = JSON.stringify(datatodownload, null, 2); // pretty print
            const blob = new Blob([jsonMsg], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
        
            const a = document.createElement('a');
            a.href = url;
            a.download = "dunbartonCameraData.json";
            a.click();
